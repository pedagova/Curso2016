...
device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	char tempBuff[len + 1];
   	int mask = 0x0;
	int i;
	if(copy_from_user(tempBuff, buff, len) != 0){
		printk(KERN_ALERT "ha ocurrido un problema en la copia");
		return -EFAULT;	
	}
	if(len > BUF_LEN) return -ENOSPC;
	
	for(i = 0; i < len; i++){
		if(tempBuff[i] == '1') mask = (mask | 0x2);
		else if(tempBuff[i] == '2') mask = (mask | 0x4);
		else if(tempBuff[i] == '3') mask = (mask | 0x1);
		else if(tempBuff[i] == '\n') break;
		else return -EINVAL;
	}
