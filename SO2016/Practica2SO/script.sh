...
if [ -d $TEMP_DIR ] ; then
rm -r $TEMP_DIR
echo "eliminado directorio temp"	
fi
mkdir $TEMP_DIR
...
DIFF_TRUNCATE=$(diff $TEMP_DIR/$MY_FS_H $RESL_DIR/$MY_FS_H)
if [ "$DIFF_TRUNCATE" == "" ];
...
DIFF_TEMP_DIR_TRUNCATE_FUSE_LIB_C=$(diff $TEMP_DIR/$FUSE_LIB_C $RESL_DIR/$FUSE_LIB_C) # See Link1 at bottom | Make diff
